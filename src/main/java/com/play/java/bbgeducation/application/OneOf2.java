package com.play.java.bbgeducation.application;
...
public class OneOf2<T1,T2> {
    ...
    public OneOf2(T1 option1, T2 option2){
        ...
        if (this._opt1.isEmpty() && this._opt2.isEmpty()){
            throw new IllegalArgumentException("OneOf must contain at one one option");
        }
    ...
    public static<T1, T2> OneOf2<T1, T2> option1 (final T1 option){
        return new OneOf2<>(option, null);
    }

    public static<T1, T2> OneOf2<T1, T2> option2 (final T2 option){
    ...
    public <R> R fold( final Function<T1,R> mapper1, final Function<T2,R> mapper2)
    {
